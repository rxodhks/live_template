...
export const ACTIVITY: Record<ActivityType, ActivityDef> = {
  ...
  'template.share': { module: 'template', label: '협업 전환', text: () => '개인 공간을 협업 공간으로 전환했습니다' },
  'template.trash': { module: 'template', label: '휴지통', text: () => '템플릿을 휴지통으로 옮겼습니다', important: true, toastKind: 'danger' },
  'template.restore': { module: 'template', label: '복원', text: () => '휴지통에서 템플릿을 복원했습니다', important: true, toastKind: 'success' },
  'template.copy': { module: 'template', label: '버전 복원', text: (t, d) => `${q(t)}템플릿의 이전 버전으로 사본을 만들었습니다${d ? ` · ${d}` : ''}` },
  'invite.create': { module: 'member', label: '초대 링크', text: (_t, d) => `초대 링크를 만들었습니다${d ? ` · ${d}` : ''}` },
  'invite.revoke': { module: 'member', label: '초대 취소', text: (_t, d) => `초대 링크를 취소했습니다${d ? ` · ${d}` : ''}` },
  'member.request': { module: 'member', label: '참여 요청', text: () => '초대 링크로 참여를 요청했습니다', important: true, toastKind: 'info' },
